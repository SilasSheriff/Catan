...
@Getter
@Setter
public class Road extends Pieces{
    ...
    public Road(Point2D.Double center, PlayerColours colour) {
        super(center, colour);
        this.center = center;
        calculateRoadCorners();

        price.put(Resources.CLAY,1);
        price.put(Resources.WOOD,1);
    }

    private void calculateRoadCorners(){
        corners.add(new Point2D.Double(center.x + 3* size, center.y + .5 * size ));
        corners.add(new Point2D.Double(center.x + 3* size, center.y - .5 * size ));
        corners.add(new Point2D.Double(center.x - 3* size, center.y - .5 * size ));
        corners.add(new Point2D.Double(center.x - 3* size, center.y + .5 * size ));
    }

    public void drawRoad(Graphics2D graph){
